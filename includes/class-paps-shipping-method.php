        ...
        class MP_Paps_Shipping_Method extends WC_Shipping_Method
        {
            ...
            public function calculate_shipping($package = array())
            {
                ...
                if (!get_option('mp_paps_enabled', false)) {
                    return; // Silent fail si désactivé
                }
                ...
                // 3. Préparer les détails des colis (Poids/Dimensions)
                $size_details = array();
                $has_packages = false;
                ...
                // Log pour débogage
                error_log('[PAPS SHIPPING] Calcul pour: ' . $destination_address . ' | Colis: ' . count($size_details));
                ...
                if (is_wp_error($result)) {
                    ...
                    // wc_add_notice(__('Erreur de calcul de livraison PAPS: ', 'mp-creator-notifier') . $result->get_error_message(), 'error');
                    return; 
                }
                ...
                if (isset($result['price']) && $result['price'] > 0) {
                    $rate = array(
                        ...
                        'meta_data' => array(
                            ...
                            __('Type colis', 'mp-creator-notifier') => $result['packageSize'],
                        ),
                    );

                    // Enregistrer le taux auprès de WooCommerce
                    $this->add_rate($rate);
                    
                    error_log('[PAPS SHIPPING] Taux ajouté: ' . $result['price']);
                }
            }
        }
    }
